...
function renderForm(signup){const f=$('fgForm');if(!f)return;f.innerHTML=signup?`<input id="fgName" placeholder="display name" autocomplete="name"><input id="fgUser" placeholder="username" autocomplete="username"><input id="fgPass" placeholder="password" type="password" autocomplete="new-password"><button id="fgCreate" class="primary" type="button">create account</button><button id="fgGoogle" class="google" type="button">continue with Google</button><p class="fg-note">Usernames are checked in Firebase.</p>`:`<input id="fgUser" placeholder="username" autocomplete="username"><input id="fgPass" placeholder="password" type="password" autocomplete="current-password"><button id="fgLogin" class="primary" type="button">sign in</button><button id="fgGoogle" class="google" type="button">continue with Google</button>`;$('fgGoogle').onclick=googleLogin;if(signup){$('fgCreate').onclick=signupLocal}else{$('fgLogin').onclick=loginLocal;$('fgPass').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();loginLocal()}})}}
function hideWall(){$('fgLoginWall')?.remove();document.body.classList.remove('fg-locked')}
function error(t){const e=$('fgError');if(e)e.textContent=t}
function showSession(u){
  // Floating bottom-right session widget removed. The real profile UI lives in shell.js topbar/sidebar.
  const old=$('fgSession');
  if(old) old.remove();
}
function showNotice(text){let n=$('fgNotice');if(!n){n=document.createElement('div');n.id='fgNotice';n.className='fg-notice';document.body.appendChild(n)}n.textContent=text;n.style.display='block';clearTimeout(window.__fgN);window.__fgN=setTimeout(()=>n.style.display='none',5200)}
function loadScript(src){return new Promise((res,rej)=>{if(document.querySelector('script[src="'+src+'"]'))return res();const s=document.createElement('script');s.src=src;s.onload=res;s.onerror=rej;document.head.appendChild(s)})}
async function initFirebase(){if(firebaseReady)return firebaseReady;firebaseReady=(async()=>{await loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-app-compat.js');await loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-database-compat.js');try{await loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore-compat.js')}catch(e){console.warn('Firestore compat not loaded',e)}try{await loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-auth-compat.js')}catch{}if(!firebase.apps.length)firebase.initializeApp(firebaseConfig);db=firebase.database();try{auth=firebase.auth()}catch{}return db})();return firebaseReady}
...
function showModerationPage(kind,u={},data={}){
  ...
  div.innerHTML=`<div class="fg-mod-card"><div class="fg-mod-head">${esc(title)}</div><div class="fg-mod-body"><p>${esc(desc)}</p><div class="fg-mod-reason"><b>Reason:</b><br>${esc(reason)}</div><p class="fg-note">Username: ${esc(u.username||'unknown')}</p></div><div class="fg-mod-actions"><button class="secondary" id="fgModLogout" type="button">Log Out</button>${kind==='warning'?'<button id="fgModOk" type="button">I Understand</button>':''}</div></div>`;
...
async function finish(raw){let u=normalizeUser(raw);if(!u||!u.uid)return;if(u.role==='banned'||Number(u.bannedUntil||0)>Date.now()){storeLocal(u);await initFirebase();showModerationPage('ban',u,{title:u.banTitle||'Account Banned',description:u.banDescription||'You may not use GameHub right now.',reason:u.banReason||'No reason provided.'});return}if(Number(u.timeoutUntil||0)>Date.now()){showWall('signin','This account is timed out.');return}storeLocal(u);hideWall();showSession(u);await initFirebase();const userPresencePatch={lastSeen:Date.now(),currentPage:pageName()};
...
function boot(){if(booted)return;booted=true;injectStyles();const cached=readLocal();if(cached&&cached.uid){window.FGUser=cached;showSession(cached);initFirebase().then(async()=>{ensureNovaBot();try{const snap=await db.ref('siteUsers/'+cached.uid).get();await finish(snap.exists()?normalizeUser({...cached,...snap.val()}):cached)}catch(e){console.warn(e);showWall('signin','Could not verify your login. Please sign in again.')}})}else{showWall('signin');initFirebase().then(()=>ensureNovaBot()).catch(()=>{})}}
